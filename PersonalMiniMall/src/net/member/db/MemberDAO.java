...
public class MemberDAO {
	...
	public int userCheck(String id, String pw) throws SQLException{
		...
		try{
			con = ds.getConnection();
			sql="select MEMBER_PW from SHOP_member where MEMBER_ID=?";
			pstmt=con.prepareStatement(sql);
	...
	public int confirmId(String id) throws SQLException{
		...
		try{
			con = ds.getConnection();
			sql="select MEMBER_ID from SHOP_member where MEMBER_ID=? ";
	...
	public MemberBean getMember(String id) throws SQLException{
		...
		System.out.println(id + "<-- id getMember MemberDAO.java");
		MemberBean member=null;
		String sql=null;
		
		try{
			con = ds.getConnection();
			sql="select * from SHOP_member where MEMBER_ID=?";
			pstmt=con.prepareStatement(sql);
			pstmt.setString(1, id);
			rs=pstmt.executeQuery();
	...
	public void updateMember(MemberBean mb) throws SQLException{
		...
		try{
			con = ds.getConnection();
			sql="update SHOP_member set MEMBER_PW=?,MEMBER_NAME=?,"+
	...
	throws SQLException{
		...
		try{
			...
			sql="select MEMBER_ID, MEMBER_PW, MEMBER_JUMIN1,"+
				"MEMBER_JUMIN2 from SHOP_member where MEMBER_NAME=?";
	...
	public boolean isAdmin(String id){
		String sql="select MEMBER_ADMIN from SHOP_member where MEMBER_ID=?";
		int member_admin=0;
		boolean result = false;
		try {
			con = ds.getConnection();
