...
<%
	...
	System.out.println(ordertype + "<-- ordertype goods_buy.jsp");
	
	if (ordertype.equals("goods")) {
		orderinfo = (ArrayList) request.getAttribute("orderinfo");
		
		System.out.println(orderinfo.size() + "<-- orderinfo.size() goods_buy.jsp");

		
	} else {
		basketlist = (ArrayList) request.getAttribute("basketlist");
		goodslist = (ArrayList) request.getAttribute("goodslist");
		System.out.println(basketlist.size() + "<-- basketlist.size() goods_buy.jsp");
		System.out.println(goodslist.size() + "<-- goodslist.size() goods_buy.jsp");
	}
...
<tr>
	...
	<!-- 주문 페이지 -->
	<form action="<%= request.getContextPath() %>/goods_order/OrderAdd.or" method="post" name="orderform">
	...
	<table border=0 cellspacing=1 cellpadding=0 width=80%>
		...
		<td style="font-family: Tahoma; font-size: 7pt;">
			<img src="<%= request.getContextPath() %>/upload/<%=goods.getGOODS_IMAGE().split(",")[0] %>" 
				width=50 height=50>
		</td>
