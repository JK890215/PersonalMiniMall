...
<table width="960" cellspacing="0" cellpadding="0" border="0"
	...
	<tr>
		<td colspan=2><!-- 장바구니 -->
		<p align="center">
		<form action="<%= request.getContextPath() %>/goods_order/OrderStart.or" name="basketform" method="post">
		...
		<table width="80%" cellpadding="0" cellspacing="0">
			...
			<td><font size="2"><img 
				src="<%= request.getContextPath() %>/upload/<%=goods.getGOODS_IMAGE().split(",")[0] %>" 
			...
			<td><font size="2">
			<a href="<%= request.getContextPath() %>/goods/BasketDelete.ba?num=<%=dto.getBASKET_NUM()%>"
		...
		<table width="80%" cellpadding="0" cellspacing="0">
			...
			<%}
			if (item == null) {%>
			<a href="<%= request.getContextPath() %>/goods/GoodsList.go?item=new_item">
			[계속 쇼핑하기]</a>
			<%}else{%>
			<a href="<%= request.getContextPath() %>/goods/Goods_Detail.go?item=<%=item %>
