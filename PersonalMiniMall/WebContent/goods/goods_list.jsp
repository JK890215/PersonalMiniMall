...
function searchPrice(item,searchprice) {
	if (searchprice!="all") {
		window.location.href="<%=request.getContextPath() %>/goods/GoodsList.go?item="+item+"&page=1&searchprice="+searchprice;
	} else {
		window.location.href="<%=request.getContextPath() %>/goods/GoodsList.go?item="+item+"&page=1";
	}
}
</script>
</head>
		...
		<tr>	   
		   <c:choose> 
				...
				<%if(category.equals("new_item")) { 
			      	if(price.equals("no")) {
				%>
				<a href="<%=request.getContextPath() %>/goods/Goods_Detail.go?item=${item.GOODS_CATEGORY}
				...
				%>
			 	...
			 	<img src="<%=request.getContextPath() %>/upload/${fn:trim(item.GOODS_IMAGE)}" 
