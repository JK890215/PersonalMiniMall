...
function isBuy(goodsform) {
	...
	if(isbuy==true) {
		goodsform.action="<%= request.getContextPath()%>/goods_order/OrderStart.or";
		goodsform.submit();
	} else {
		return;
	}
...
function isBasket(basketform) {
	...
	if(isbuy==true) {
		basketform.action="<%= request.getContextPath() %>/goods/BasketAdd.ba";
		basketform.submit();
	} else {
		return;
	}
...
<table width="960" cellspacing="0" cellpadding="0" border="0"
	...
	<tr>
		...
		<tr>
			...
			<img src="<%=request.getContextPath() %>/upload/${fn:trim(mainImage)}" 
			...
			<table align="center">
				<tr>
					...
					if (prevpage.getGOODS_NUM() != 0) {
						if (price.equals("no")) {
					%>
						<a href="<%=request.getContextPath() %>/goods/Goods_Detail.go?search=prev
					...
					<div align="center">
						<img 
						src="<%=request.getContextPath() %>/upload/<%=prevpage.getGOODS_IMAGE()%>"
					...
					%>
						<div align="center">
						<img
						src="<%=request.getContextPath() %>/upload/<%=nextpage.getGOODS_IMAGE()%>"
						width="70" height="50" border="0">
					...
					<%
						if (price.equals("no")) {
					%>
						<a href="<%=request.getContextPath() %>/goods/Goods_Detail.go?search=next
						&gr_goods_num=<%=itemArray.getGOODS_NUM()%>
						&item=<%=category%>">
					<%
						} else {
		...
		<table width="200" border="0" align="center">
			<tr align="left">
				<td colspan="3"><c:forEach var="itemimg"
					...
					<img src="<%=request.getContextPath() %>/upload/${fn:trim(itemimg)}" />
				</c:forEach></td>
			</tr>
		</table>
		<!-- 상품 내용 끝 --></td>
